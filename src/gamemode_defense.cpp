...
void defense_game::init_map()
{
    ...
    std::string defloc_special_id;
    switch( location ) {
        case DEFLOC_NULL:
        case NUM_DEFENSE_LOCATIONS:
            defloc_special_id = "house_two_story_basement";
            DebugLog( D_ERROR, D_GAME ) << "defense location is invalid: " << location;
            break;

        case DEFLOC_HOSPITAL:
            defloc_special_id = "hospital";
            break;

        case DEFLOC_WORKS:
            defloc_special_id = "public_works";
            break;

        case DEFLOC_MALL:
            defloc_special_id = "megastore";
            break;

        case DEFLOC_BAR:
            defloc_special_id = "bar";
            break;

        case DEFLOC_MANSION:
            defloc_special_id = "Mansion_Wild";
            break;
    }
    const om_direction::type defloc_rotation = om_direction::random();
    defloc_special = overmap_specials::get_specific( defloc_special_id );
    starting_om.place_special_forced( defloc_special, defloc_pos, defloc_rotation );
