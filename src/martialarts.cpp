    ...
    if( !ma_style.str().empty() ) {
        ...
        if( !valid_ma_weapons.empty() ) {
            ...
            for( auto &weaps : weaps_by_cat ) {
                ...
                weapons_text.emplace( std::make_pair( w_cat, enumerate_as_string( weaps.second ) ) );
            }
            if( has_other_cat ) {
                ...
                weapons_text.emplace( std::make_pair( _( "OTHER" ), enumerate_as_string( weaps ) ) );
            }
        }
    }
}
...
void ma_details_ui_impl::draw_ma_details_text() const
{
    ...
    if( !general_info_text.empty() &&
        ...
        for( const std::string &entry : general_info_text ) {
    ...
    if( !weapons_text.empty() &&
        ...
        for( const std::pair<std::string, std::string> &entry : weapons_text ) {
...
void show_ma_details_ui( const matype_id &style_selected )
{
